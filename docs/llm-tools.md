...
`npm run test:professor-context-tool-flow` executa 125 testes offline do schema, fluxo e rota. Eles cobrem game, position e ausência de Tool; executor correto e exclusão do outro; minimização da primeira entrada; matriz completa de incompatibilidade; protocolo e argumentos; guards estruturais de output; variantes reais do `ParsedResponse`; metadados adicionais ignorados; output não-array; diagnóstico sanitizado; preservação e ordem; serialização única; configuração das duas interações; falhas e ausência de vazamento; formato e limites do ID de posição; limite real e declarado do body; precedência antes do cliente; wiring; sucessos das três branches; mapeamento HTTP; logs sanitizados; restauração de ambiente e execução serial dos testes que alteram `process.env`.

O eval conjunto executa 57 testes offline: 52 do runner e da CLI, além de cinco da definição canônica. O agregado `npm run test:ai-tools` contém 478 testes únicos após a preparação da comparação de prompts da Etapa 7F-A. A suíte específica de prompts possui outros quatro testes e não integra esse agregado. Suítes específicas e agregada se sobrepõem e não devem ser somadas novamente.
...
A matriz server-side continua sendo a autoridade final. Uma escolha errada do modelo não concede acesso, não troca o contexto autorizado e não cria fallback. A primeira execução real do runner ocorreu em `E-022`, mas foi inconclusiva por falha de integração: todas as 12 execuções terminaram como erro técnico, sendo dez `FINAL_RESPONSE_OUTPUT_INVALID` e duas `TOOL_CONTEXT_MISMATCH`. `decisionAccuracy` ficou `null` e `completionRate`, `0`; isso não é accuracy de 0% e não fornece evidência sobre a qualidade do modelo.

### Correção da integração após E-022

O SDK OpenAI 6.47.0 declara `ParsedResponseOutputText<ParsedT>` como extensão de `ResponseOutputText` com o campo adicional `parsed`. O validador anterior reimplementava os tipos como objetos de chaves fechadas e rejeitava esse metadado legítimo antes de chegar à validação de `response.output_parsed`. A falha era, portanto, do tratamento do output bruto do SDK, não uma reprovação do Structured Output público: em dez casos havia resposta final, latências das duas interações e `usage`, mas o pipeline a interrompeu antes de aceitar ou rejeitar o payload público pelo schema local.

O fluxo agora valida os discriminadores e os campos estruturais efetivamente consumidos em `reasoning`, `message`, `output_text`, `refusal` e `function_call`. Propriedades adicionais legítimas são ignoradas, nunca copiadas para o resultado e nunca usadas como substituto de `output_parsed`. Tipos desconhecidos, objetos malformados, refusal, resposta incompleta e `function_call` final continuam proibidos. Somente `provisionalTeacherResponseSchema.safeParse(response.output_parsed)` pode produzir `data` público.
