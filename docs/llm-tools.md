...
O eval conjunto executa 54 testes offline: 37 do runner, 12 da CLI e cinco da definição canônica. O agregado `npm run test:ai-tools` contém 475 testes únicos após a correção da Etapa 7C-D. Suítes específicas e agregada se sobrepõem e não devem ser somadas novamente.
