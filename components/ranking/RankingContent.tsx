...
const initialFilters: RankingFilterState = { query: "", region: "all", ratingRange: "all", sort: "official" };
...
export function RankingContent() {
  ...
  const visiblePlayers = useMemo(() => {
    ...
    const filtered = officialPlayers.filter((player) => (!query || normalize(`${player.name} ${player.region}`).includes(query)) && (filters.region === "all" || player.region === filters.region) && matchesRatingRange(player.currentPlatformRating, filters.ratingRange));
  ...
  const hasFilters = filters.query !== "" || filters.region !== "all" || filters.ratingRange !== "all";
