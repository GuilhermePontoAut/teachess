...
import { formatPercent, getInitials } from "./ranking";
...
function PodiumPlayerCard({ player, visualIndex }: { player: RankingPlayer; visualIndex: number }) {
  return <article className={`relative rounded-2xl border-2 bg-white p-5 shadow-sm ${podiumStyles[visualIndex]}`}><div className="flex items-start justify-between gap-3"><span className="flex size-12 items-center justify-center rounded-full bg-neutral-950 text-sm font-bold text-white" aria-hidden="true">{getInitials(player.name)}</span><span className="rounded-full border border-line px-2.5 py-1 text-xs font-bold">{medals[visualIndex]}</span></div><h3 className="mt-4 text-lg font-semibold">{player.name}</h3><p className="text-sm text-muted">{player.region}</p><div className="mt-4 grid grid-cols-3 gap-2 border-t border-line pt-4 text-sm"><div><p className="text-xs text-muted">Rating</p><p className="font-bold tabular-nums">{player.currentPlatformRating}</p></div><div><p className="text-xs text-muted">Taxa de vitória</p><p className="font-bold tabular-nums">{formatPercent(player.winRate)}</p></div><div><p className="text-xs text-muted">Partidas</p><p className="font-bold tabular-nums">{player.platformGames}</p></div></div></article>;
}
