...
export function RatingComparison({ ratingAtGame, currentRating, label, externalContext = false, compact = false }: RatingComparisonProps) {
  const difference = currentRating === undefined ? undefined : currentRating - ratingAtGame;
  ...
  const differenceText = difference === undefined ? "Rating atual não disponível" : difference === 0 ? "Sem alteração" : `${difference > 0 ? "+" : ""}${difference} pontos desde esta partida`;

  if (compact) return <div className={`rounded-lg border px-2.5 py-2 text-xs ${tone}`} aria-label={`${label}: rating na partida ${ratingAtGame}. ${differenceText}.`}><span className="font-semibold">{ratingAtGame}</span><Icon className="mx-1 inline" size={13} aria-hidden="true" />{currentRating ?? "—"}<span className="ml-1 font-medium">({difference === undefined ? "indisponível" : difference === 0 ? "0" : `${difference > 0 ? "+" : ""}${difference}`})</span></div>;

  return <section className={`rounded-2xl border p-4 ${tone}`} aria-label={`Comparação de rating de ${label}`}>
    <p className="text-sm font-semibold">{label}</p>
    <dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs opacity-75">Rating na partida</dt><dd className="mt-1 text-lg font-bold">{ratingAtGame}</dd></div><div><dt className="text-xs opacity-75">Rating atual no TeaChess</dt><dd className="mt-1 text-lg font-bold">{currentRating ?? "Não disponível"}</dd></div></dl>
