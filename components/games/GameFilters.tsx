...
export function GameFilters({ filters, openings, onChange, onClear }: { filters: GameFilterState; openings: string[]; onChange: (changes: Partial<GameFilterState>) => void; onClear: () => void }) {
  return <section aria-labelledby="game-filters-title" className="rounded-2xl border border-line bg-surface p-4 shadow-sm sm:p-5">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      ...
      <Select label="Ordenar por" value={filters.sort} onChange={(value) => onChange({ sort: value as GameFilterState["sort"] })} options={[["newest","Data mais recente"],["oldest","Data mais antiga"],["rating-desc","Maior rating adversário"],["rating-asc","Menor rating adversário"],["moves-desc","Maior quantidade de lances"],["moves-asc","Menor quantidade de lances"],["accuracy-desc","Maior precisão"],["accuracy-asc","Menor precisão"]]} />
    </div>
