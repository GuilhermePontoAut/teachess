...
function SummaryItem({ label, value, icon: Icon, description }: SummaryMetric) {
  return <div className="flex h-full min-h-36 min-w-0 flex-col rounded-2xl border border-line bg-surface p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><span className="text-sm font-medium text-muted">{label}</span><Icon size={17} className="shrink-0 text-neutral-500" aria-hidden="true" /></div><p className="mt-2 text-2xl font-bold text-neutral-950">{value}</p>{description && <p className="mt-2 break-words text-xs leading-5 text-muted">{description}</p>}</div>;
}
