...
import { canDeleteGame } from "@/lib/utils/gameRules";
...
export function GamesContent({ initialMessage = "" }: { initialMessage?: string }) {
  ...
  const hasFilters = filters.query !== "" || filters.origin !== "all" || filters.result !== "all" || filters.color !== "all" || filters.analysis !== "all" || filters.opening !== "all";
  ...
  return <div className="space-y-7">
    ...
    <DeleteGameDialog open={gameToDelete !== null && canDeleteGame(currentUser, gameToDelete)} title="Excluir partida?" description={`A partida contra ${gameToDelete?.opponent ?? "este adversário"} será removida deste navegador. Esta ação não pode ser desfeita.`} confirmLabel="Excluir partida" destructive onCancel={() => setGameToDelete(null)} onConfirm={() => { if (gameToDelete && canDeleteGame(currentUser, gameToDelete)) { deleteGame(gameToDelete.id); setMessage(`Partida contra ${gameToDelete.opponent} excluída.`); } setGameToDelete(null); }} />
