...
export function GameActions({ game, onDelete }: { game: ChessGame; onDelete: (game: ChessGame) => void }) {
  return <div className="flex flex-wrap items-center gap-1" aria-label={`Ações para partida contra ${game.opponent}`}><Link href={`/partidas/${game.id}`} className={actionClass}><Eye size={16} aria-hidden="true" />Ver detalhes</Link>{canEditGameNotes(currentUser, game) && <Link href={`/partidas/${game.id}/editar`} className={actionClass}><Pencil size={16} aria-hidden="true" />Editar observações</Link>}<Link href={`/partidas/${game.id}/analise`} className={actionClass}><BarChart3 size={16} aria-hidden="true" />Abrir análise</Link>{canDeleteGame(currentUser, game) && <button type="button" onClick={() => onDelete(game)} className={`${actionClass} text-red-700`}><Trash2 size={16} aria-hidden="true" />Excluir</button>}</div>;
}
