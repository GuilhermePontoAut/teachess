...
interface MatchSetupProps {
  players: AvailablePlayer[]; controls: TimeControl[]; opponent: AvailablePlayer | null; timeControl: TimeControl | null;
  colorChoice: DemoColorChoice | null; randomIndex: number; hasActiveMatch: boolean; onContinue: () => void;
  onOpponent: (player: AvailablePlayer) => void; onTimeControl: (control: TimeControl) => void;
  onColorChoice: (choice: DemoColorChoice) => void; onStart: (config: DemoMatchConfig) => StartMatchResult;
}

export function MatchSetup({ players, controls, opponent, timeControl, colorChoice, randomIndex, hasActiveMatch, onContinue, onOpponent, onTimeControl, onColorChoice, onStart }: MatchSetupProps) {
  ...
  return <div className="space-y-6">
    {hasActiveMatch && <section role="status" aria-live="polite" className="rounded-2xl border border-amber-400 bg-amber-50 p-5 text-amber-950"><p className="font-semibold">{blockedReason}</p><button type="button" onClick={onContinue} className={`${buttonBase} mt-3 bg-neutral-950 text-white`}>Continuar partida</button></section>}
    <section className="rounded-2xl border border-line bg-surface p-5 shadow-sm sm:p-6"><AvailablePlayers players={players} selectedId={opponent?.id ?? null} disabled={hasActiveMatch} onSelect={onOpponent}/></section>
