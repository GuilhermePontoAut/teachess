...
export function PlayerCard({ player, selected, ratingAccepted, disabled, onSelect }: { player: AvailablePlayer; selected: boolean; ratingAccepted: boolean; disabled: boolean; onSelect: () => void }) {
  ...
  return <article className={`rounded-2xl border p-4 transition ${selected ? "border-neutral-950 bg-neutral-100 ring-1 ring-neutral-950" : "border-line bg-white"}`}>
    ...
    <div className="mt-4 border-t border-line pt-3"><p className="text-xs text-muted">Prefere {player.preferredTime}</p><p className="mt-1 text-xs text-muted">Aceita desafios entre {min} e {max} de rating.</p><button type="button" disabled={disabled || !available || !ratingAccepted} onClick={onSelect} aria-pressed={selected} title={disabled ? "Encerre a partida em andamento antes de selecionar outro adversário." : undefined} className={`${buttonBase} mt-3 w-full border border-line bg-white py-2 hover:bg-neutral-100`}>{selected ? "Selecionado" : disabled ? "Partida em andamento" : !available ? "Indisponível" : ratingAccepted ? "Selecionar" : "Fora da faixa aceita"}</button></div>
  </article>;
}
