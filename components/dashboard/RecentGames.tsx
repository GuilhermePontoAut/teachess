...
export function RecentGames({ games }: { games: ChessGame[] }) {
  return (
    <section aria-labelledby="recent-games-title" className="overflow-hidden rounded-2xl border border-line bg-surface shadow-sm">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[760px] text-left text-sm">
          ...
          <tbody className="divide-y divide-neutral-200">
            {games.map((game) => <tr key={game.id} className="text-neutral-700"><td className="px-6 py-4 font-semibold text-neutral-900">{game.opponent}</td><td className="whitespace-nowrap px-4 py-4">{formatDate(game.date)}</td><td className="px-4 py-4">{colorLabels[game.playerColor]}</td><td className="px-4 py-4"><span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${resultStyles[game.result]}`}>{resultLabels[game.result]}</span></td><td className="max-w-52 px-4 py-4">{game.opening}</td><td className="whitespace-nowrap px-6 py-4 text-right font-medium">{game.accuracy === null ? "Pendente" : `${game.accuracy.toFixed(1)}%`}</td></tr>)}
          </tbody>
        </table>
      </div>
    </section>
