...
export function HumanTeachersContent() {
  ...
  return <div className="space-y-7">
    ...
    {success && <div role="alertdialog" aria-modal="true" aria-labelledby="booking-success-title" className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><CheckCircle2 size={32}/><h2 id="booking-success-title" className="mt-4 text-xl font-semibold">Agendamento simulado salvo</h2><p className="mt-2 text-sm leading-6 text-muted">A aula online com {success.teacherName} foi registrada apenas neste navegador. Nenhuma videochamada, reserva real, mensagem ou pagamento foi criado.</p><button type="button" autoFocus onClick={() => { setSuccess(null); setTab("bookings"); }} className="mt-5 min-h-11 rounded-xl bg-neutral-950 px-4 text-sm font-semibold text-white">Ver meus agendamentos</button></div></div>}
