...
import { defaultAllowContextLookup } from "@/lib/future-ai/data-access";
...
export function AiProfessorDemo() {
  ...
  const [hydrated, setHydrated] = useState(false); const [restored, setRestored] = useState(false); const [context, setContext] = useState<FutureAiContextRef | null>(null); const [allowContextLookup, setAllowContextLookup] = useState(false); const [question, setQuestion] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(false); const [confirmOpen, setConfirmOpen] = useState(false); const requestInFlight = useRef(false); const activeController = useRef<AbortController | null>(null);
    ...
    try {
      const response = await fetch("/api/ai/professor", {
        ...
        body: JSON.stringify({
          message: trimmed,
          authorizedContext,
          dataAccessPreference: { allowContextLookup },
        }),
