...
A segunda função determinística preparada é `get_game_context`. Seu runtime consulta exclusivamente o snapshot mínimo da única partida previamente selecionada e pertencente ao solicitante, com autorização owner-only por igualdade de IDs. O snapshot usa somente `requestingUserId`, sem papel do usuário, e valida origem, privacidade, metadados e PGN, diferenciando suficiência para fatos cadastrais de suficiência para consultar a sequência de lances. Partidas da plataforma continuam públicas na interface demonstrativa, mas partidas públicas de outros usuários não são enviadas ao Professor IA: visualização pública não autoriza transferir observações, tags ou outros dados ao provedor de IA. O FEN de `ChessGame` foi excluído desta Tool e permanece responsabilidade do contexto de posição, por meio de `get_position_context`. PGN ausente continua sendo uma condição normal, não um erro técnico. Esta etapa é totalmente offline: não chama a OpenAI, não registra a nova Tool na Responses API e não a conecta ao Professor IA. Os contratos e limites estão em [`docs/llm-tools.md`](docs/llm-tools.md).
...
A avaliação real da seleção automática inclui o histórico inicial `E-020` e a execução de consistência `E-021`, com três repetições de cada um dos seis casos curados sobre o mesmo snapshot. Esse resultado aumenta a evidência operacional nessa configuração, mas a baixa diversidade de mensagens, snapshots, modelos e prompts não permite generalização nem garantia estatística. Não existem RAG, integração real com a interface do Professor IA, autenticação, rate limiting do endpoint final ou validação por engine de xadrez. Os fluxos de function calling permanecem restritos a rotas técnicas. Tokens e custos ainda não foram medidos, e as latências observadas não constituem SLA ou benchmark definitivo.
...
A infraestrutura de IA possuía historicamente uma suíte agregada com 121 testes automatizados aprovados antes de `get_game_context`. A nova Tool acrescenta 73 testes offline exclusivos, levando `npm run test:ai-tools` a 194 testes únicos. Dentro do total histórico permanecem 30 testes exclusivos do runner de evals e 62 testes relacionados ao fluxo e ao runner de seleção automática de `get_position_context`. Os comandos específicos e agregados possuem sobreposição; por isso, suas quantidades não devem ser somadas como testes únicos. A validação atual também registra lint, build de produção e `git diff --check` aprovados.
