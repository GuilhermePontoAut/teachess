...
A infraestrutura de IA possuía historicamente uma suíte agregada com 121 testes automatizados aprovados antes de `get_game_context`. O runtime determinístico da partida executa 75 testes, e a definição OpenAI, o fluxo forçado e a rota técnica de partida executam 96 testes offline. A revisão de segurança da Etapa 7C-A elevou sua suíte específica para 119 testes e acrescentou cobertura do formato do ID aos baselines de posição, levando `npm run test:ai-tools` a 415 testes únicos. Dentro do total permanecem os testes do runner e da seleção automática de `get_position_context`; os comandos específicos e agregados possuem sobreposição e, por isso, suas quantidades não são somadas novamente. Nenhum teste desta etapa alcança a rede ou a OpenAI.
