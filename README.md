...
Sobre o fluxo conjunto já implementado, a Etapa 7C-B preparou um segundo runner opt-in para medir a escolha entre `get_game_context`, `get_position_context` e nenhuma Tool. O eval set possui 12 casos canônicos sintéticos, com quatro casos por decisão, e será executado sequencialmente. O runner ainda não foi executado, portanto `E-022` permanece `not_executed` e nenhuma conclusão real foi obtida. Um futuro relatório sanitizado será gravado somente em `/tmp/teachess-professor-context-tool-selection-evals.json`; tokens serão registrados quando o SDK fornecer `usage`, sem estimativas, e custo financeiro não será calculado.

`wrong_tool` existe na taxonomia do runner e pode ser exercitado offline por executores injetados. No fluxo real, porém, a matriz server-side bloqueia uma Tool incompatível com o contexto autorizado antes da execução; o resultado observável é `TOOL_CONTEXT_MISMATCH` classificado como `technical_error`, não uma decisão pública `wrong_tool`. Essa barreira protege a autorização, mas limita a observabilidade direta de algumas células da matriz de confusão. `E-022` continua `not_executed`.
...
A avaliação real da seleção automática inclui o histórico inicial `E-020` e a execução de consistência `E-021`, com três repetições de cada um dos seis casos curados sobre o mesmo snapshot. Esse resultado aumenta a evidência operacional nessa configuração, mas a baixa diversidade de mensagens, snapshots, modelos e prompts não permite generalização nem garantia estatística. Não existem RAG, integração real com a interface do Professor IA, autenticação, rate limiting do endpoint final ou validação por engine de xadrez. Os fluxos de function calling permanecem restritos a rotas técnicas. Tokens e custos não foram medidos em `E-020` ou `E-021`; o runner de `E-022` poderá registrar tokens fornecidos pelo SDK, mas não calculará custo. As latências observadas não constituem SLA ou benchmark definitivo.
...
npm run test:professor-context-tool-selection-evals # 50 testes offline do eval conjunto
...
A infraestrutura de IA possuía historicamente uma suíte agregada com 121 testes automatizados aprovados antes de `get_game_context`. O runtime determinístico da partida executa 75 testes, e a definição OpenAI, o fluxo forçado e a rota técnica de partida executam 96 testes offline. A revisão de segurança da Etapa 7C-A elevou sua suíte específica para 119 testes e acrescentou cobertura do formato do ID aos baselines de posição. A revisão final da Etapa 7C-B eleva a suíte específica do eval conjunto a 50 testes, incluindo subtestes agrupados de adulteração semântica, e leva `npm run test:ai-tools` a 465 testes únicos. Dentro do total permanecem as suítes específicas, os runners e a seleção automática; os comandos específicos e agregados se sobrepõem e não são somados novamente. Nenhum teste desta etapa alcança a rede ou a OpenAI.
...
O novo conjunto `E-022` também conserva `status: "not_executed"`: nesta etapa somente sua infraestrutura e seus testes offline foram preparados. Não houve chamada ao modelo, resultado pré-preenchido nem accuracy real calculada.
