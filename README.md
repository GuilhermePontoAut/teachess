...
A infraestrutura de IA possuía historicamente uma suíte agregada com 121 testes automatizados aprovados antes de `get_game_context`. O runtime determinístico da partida agora executa 75 testes, e a definição OpenAI, o fluxo forçado e a rota técnica executam 96 testes offline, levando `npm run test:ai-tools` a 292 testes únicos. Dentro do total permanecem 30 testes exclusivos do runner de evals e 62 testes relacionados ao fluxo e ao runner de seleção automática de `get_position_context`. Os comandos específicos e agregados possuem sobreposição; por isso, suas quantidades não são somadas novamente como testes únicos. Nenhum teste desta etapa alcança a rede ou a OpenAI.
