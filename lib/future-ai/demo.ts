...
export type FutureAiContextType = "game" | "analysis" | "position" | "training" | "none";
export type FutureAiTab = "professor" | "capabilities" | "architecture" | "roadmap";

export interface FutureAiContextRef { type: FutureAiContextType; id: string | null; label: string; }
...
export const contextTypeLabels: Record<FutureAiContextType, string> = { game: "Partida", analysis: "Análise de partida", position: "Posição salva por imagem", training: "Tema de treinamento", none: "Sem contexto específico" };
...
const questions: Record<FutureAiContextType, string[]> = {
  game: ["Onde comecei a perder vantagem?", "Qual foi meu principal erro?", "O que fiz bem nesta partida?", "Que tema devo estudar?", "Explique esta partida para um iniciante."],
  analysis: ["Qual padrão aparece nesta análise?", "Como transformar os erros em estudo?", "O que fiz bem nesta partida?", "Qual momento merece revisão?", "Explique a recomendação para um iniciante."],
  position: ["Quais são os planos desta posição?", "Quais peças estão mal posicionadas?", "Existe alguma ameaça imediata?", "O que devo analisar antes de jogar?", "Como um professor explicaria esta posição?"],
  training: ["Por que este tema foi recomendado?", "Como estudar este assunto?", "Qual erro este exercício pretende corrigir?", "Qual deve ser meu próximo passo?"],
  none: ["Como revisar uma partida com método?", "Como identificar meus padrões de erro?", "Como organizar uma sessão de estudo?", "Como trabalhar com um professor humano?"],
};
...
export function createSimulatedAnswer(context: FutureAiContextRef, question: string, data: DemoContextData): SimulatedAnswerContent {
  if ((context.type === "game" || context.type === "analysis") && data.game) {
  ...
  if (context.type === "position" && data.position) {
  ...
  if (context.type === "training" && data.training) return { summary: `O tema “${data.training.title}” faz parte do catálogo mockado de treinamento. A prioridade desta demonstração usa dados existentes e não foi escolhida por IA.`, observations: [data.training.description, `Progresso local: ${Number.isFinite(data.training.progress) ? data.training.progress : 0}%.`, `Categoria: ${errorCategoryLabels[data.training.category]}.`], learning: includes(question, ["por que", "recomendado"]) ? "Recomendações futuras deverão mostrar quais partidas, erros e regras sustentam a escolha." : "Prática curta e revisão espaçada ajudam a tornar o objetivo observável.", plan: ["Rever o conceito em 5 minutos.", `Resolver uma pequena série de exercícios de ${data.training.title.toLocaleLowerCase("pt-BR")}.`, "Registrar um erro e um acerto para a próxima sessão."], nextQuestion: "Como vou medir se este tema melhorou?", contextUsed: `Treinamento privado: ${data.training.title}` };
