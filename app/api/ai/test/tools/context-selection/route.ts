...
export function flowErrorStatus(code: ProfessorContextToolFlowErrorCode): number {
  switch (code) {
    ...
    try {
      ...
      if (error instanceof UnknownProfessorIaPromptVersionError) {
        dependencies.logError(
          "[api/ai/test/tools/context-selection] prompt_version_not_configured",
        );
        return errorResponse(
      ...
      if (error instanceof MissingOpenAIApiKeyError) {
        dependencies.logError(
          "[api/ai/test/tools/context-selection] server_not_configured",
        );
        return errorResponse(
      ...
      if (error instanceof ProfessorContextToolFlowError) {
        if (error.code === "PROVIDER_ERROR") {
          ...
          dependencies.logError(
            `[api/ai/test/tools/context-selection] ${error.code}`,
          );
        }
        return errorResponse(
          error.code,
