...
import { positionContextToolRequestSchema } from "@/lib/ai/tools/position-context-tool-flow";
...
export function autoFlowErrorStatus(
...
async function handleEnabledRequest(request: Request): Promise<Response> {
  ...
  const result = await runAutoPositionContextToolFlow(
    {
      ...parsedBody.data,
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (error instanceof UnknownProfessorIaPromptVersionError) {
      console.error(
        "[api/ai/test/tools/position-context/auto] prompt_version_not_configured",
      );
      return errorResponse(
    ...
    if (error instanceof MissingOpenAIApiKeyError) {
      console.error(
        "[api/ai/test/tools/position-context/auto] server_not_configured",
      );
      return errorResponse(
    ...
    if (error instanceof AutoPositionContextToolFlowError) {
      if (error.code === "PROVIDER_ERROR") {
        console.error(
          "[api/ai/test/tools/position-context/auto] provider_error",
          getSafeOpenAIErrorDiagnostic(error.cause),
        );
      } else {
        console.error(
          `[api/ai/test/tools/position-context/auto] ${error.code}`,
        );
      }
      return errorResponse(
        error.code,
        error.message,
        autoFlowErrorStatus(error.code),
      );
    }

    console.error("[api/ai/test/tools/position-context/auto] internal_error");
    return errorResponse(
      "internal_error",
